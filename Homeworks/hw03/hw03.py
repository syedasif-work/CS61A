...
def missing_digits(n):
    ...
    if n // 10 == 0:
        ...
        ones = n % 10
        tens = (n // 10) % 10
        if ones - tens == 0 or ones - tens == 1:  # (8 - 4) = 4 - 1
            return missing_digits(n // 10)
        else:
            return ones - tens - 1 + missing_digits(n // 10)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_change_helper(total, denom):
        if total == 0:
            return 1
        if denom > total:
            return 0
        if total < 0:
            ...
            with_denom = count_change_helper(total - denom, denom)
            without_denom = count_change_helper(total, denom * 2)
            return with_denom + without_denom
