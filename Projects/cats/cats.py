...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_paras = [p for p in paragraphs if select(p)]
    if k < len(select_paras):
        return select_paras[k]
    return ""
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if (len(typed_words) == 0):
        ...
        return accuracy * 100 / len(typed_words)
    # END PROBLEM 3
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def shifty_shifts_helper(s1, s2, diff):
        if diff > limit:
            return limit + 1
        if not s1 or not s2:
            return diff + max(len(s1), len(s2))
        elif s1[0] == s2[0]:
            return shifty_shifts_helper(s1[1:], s2[1:], diff)
        else:
            return shifty_shifts_helper(s1[1:], s2[1:], diff + 1)

    # END PROBLEM 6
    result = shifty_shifts_helper(start, goal, 0)
    return result
...
def time_per_word(times_per_player, words):
    ...
    """
    def time_per_word_helper(times):
        lst = []
        for i in range(len(times) - 1):
            lst.append(times[i + 1] - times[i])
        return lst

    return game(words, [time_per_word_helper(time) for time in times_per_player])
...
def fastest_words(game):
    ...
    for word in words:
        ...
        for player in players:
            if all_times(game)[player][word] < min_value:
                min_index = player
                min_value = all_times(game)[player][word]
        lst[min_index].append(all_words(game)[word])
    return lst
...
meowstake_matches = count(meowstake_matches)
shifty_shifts = count(shifty_shifts)
